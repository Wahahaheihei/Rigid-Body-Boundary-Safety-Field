...
- `core_model.py` — numerical core for boundary sampling, dynamic influence
  construction, constrained virtual-mass estimation, ASPE/ASPE-rate, ADSI,
  and representative snapshot rendering.
- `plot_timeseries.py` — reconstructs the baseline ADSI time series from the
...
- `data/raw_aspe_timeseries.csv` — archived aircraft-level ASPE and ASPE-rate
...
- `data/normalization_reference.csv` — normalization reference values.
- `data/summary.csv` — key deterministic values.
- `reference_output/` — reference figures generated from the retained baseline
  configuration.
